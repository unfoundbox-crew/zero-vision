...
status: living
updated: 2026-09-12
verified_against: feat/warm-local-vlm
...
flowchart LR
    ...
    PERCEIVE --> LV[local-vlm: warm daemon, else one-shot runner]
    ...
    LV -->|unix socket, 0600| DAEMON[local_vlm_daemon.py, weights held]
    LV -->|fallback: spawn per call| ONESHOT[local_vlm_runner.py]
    DAEMON --> MLX[(mlx-vlm + weights on disk)]
    ONESHOT --> MLX
...
zrv/zrv-mcp --+-- CDP reader --ws--> debug Chrome (:1948, :9223)
              ...
              +-- perceive() --+-- apple-vision --spawn--> zrv-native (Swift)
                                ...
                                +-- local-vlm    --unix socket--> local_vlm_daemon.py (warm, weights held)
                                |                  --or spawn--> local_vlm_runner.py (cold fallback)
...
| local-vlm warm client | `src/engines/local-vlm-warm.ts` | Finds/starts the warm daemon, one JSON line per request over a 0600 Unix socket; every failure is a returned reason, never a throw | `socketPath()`, `daemonHealth()`, `ensureDaemon()`, `ask()`, `stopDaemon()`, `WARM_PROTOCOL` |
| local-vlm warm daemon | `src/engines/local_vlm_daemon.py` | Holds one model for many calls. Unix socket, one inference at a time, queue cap, idle exit, exclusive flock. Imports `load_model()`/`infer()` from the runner so warm and cold run identical inference | socket protocol in its own docstring: `health`, `stop`, `perceive` |
...
| `zrv local-vlm status [--json]` | Warm-daemon state: pid, pinned model, protocol, load ms, request/queue counters, idle vs idle budget. Exit 0 running, 3 none | humans, agents |
| `zrv local-vlm stop [--json]` | Asks the warm daemon to exit; removes a stale socket. Exit 0 stopped, 3 nothing to stop | humans, agents |
...
```ts
{
  ...
  model?: string;
  error?: string;
  warm?: boolean;      // local-vlm only: served by the warm daemon, no model load
  warmError?: string;  // local-vlm only: why the cold path ran instead
}
```
...
| `local-vlm` | Named explicitly | Real mlx-vlm inference. Spawns `pythonBin() local_vlm_runner.py` with JSON on stdin. `capabilities: ["transcribe","describe"]`. Weights are never downloaded: `resolveModel()` looks up an `mlx-community` HF id in the local HF cache (or takes a directory) and returns `local_vlm_no_weights` naming the path and the download command when nothing is there. Refuses video and clipboard. 30 MB cap. Warm by default since 0.2.2: the first call starts a per-user daemon (`local_vlm_daemon.py`) over a 0600 Unix socket that holds the weights; later calls skip the load. `ZRV_LOCAL_VLM_WARM=0` pins the one-shot spawn. A daemon that cannot start is not an error — the cold path runs and the result carries `warmError`. Measured 2026-09-12, one 1280x800 PNG, `mlx-community/Qwen2-VL-2B-Instruct-4bit` (1.2 GB, 4-bit) at `nice 19`, machine busy (load 24-45): **warm p50 5.7 s transcribe / 6.7 s describe** (n=5 each), against **10.8-17.4 s / 11.0-14.1 s** cold one-shot in the same session; first call 9.1 s / 13.0 s, of which 9.4 s was the model load. The earlier single-shot 13.1 s / 12.6 s figures were the same model on a quieter machine. The documented 8B default is still unmeasured |
...
- No database. Every CLI/MCP call is stateless per invocation with one exception: `local-vlm`'s warm daemon, below.
- **Warm daemon state** (the only thing that outlives a call): a Unix socket at `~/Library/Application Support/zero-vision/local-vlm.sock` on macOS, `${XDG_STATE_HOME:-~/.local/state}/zero-vision/local-vlm.sock` elsewhere, overridden whole by `ZRV_LOCAL_VLM_SOCK` (`ZRV_STATE_DIR` moves the directory). Directory `0700`, socket `0600`, owner-only; nothing binds a network port. Beside it, `local-vlm.sock.lock` carries an exclusive `flock` so exactly one daemon owns a socket — a second one exits 4 rather than racing. The daemon removes the socket on exit and leaves the lock file. It holds one model, pinned at spawn, and exits after `ZRV_LOCAL_VLM_IDLE_S` (default 600 s) with no in-flight or waiting call. No image bytes, text, or model output are written anywhere: requests and replies are one JSON line each over the socket, same shape as the cold runner's stdin/stdout.
...
- `local-vlm` env: `ZRV_LOCAL_VLM_MODEL`, `ZRV_PYTHON`, `ZRV_LOCAL_VLM_TIMEOUT_MS` (180000), `ZRV_LOCAL_VLM_MAX_TOKENS` (512), `ZRV_LOCAL_VLM_RUNNER` (runner-script seam). Warm path: `ZRV_LOCAL_VLM_WARM` (1), `ZRV_LOCAL_VLM_IDLE_S` (600) / `ZRV_LOCAL_VLM_IDLE_MS` (wins when set), `ZRV_LOCAL_VLM_QUEUE_MAX` (2), `ZRV_LOCAL_VLM_SOCK`, `ZRV_STATE_DIR`, `ZRV_LOCAL_VLM_NICE` (19), `ZRV_LOCAL_VLM_SPAWN_TIMEOUT_MS` (20000), `ZRV_LOCAL_VLM_DAEMON` (daemon-script seam). HF cache location honours `HF_HUB_CACHE` / `HF_HOME`.
...
- Nothing is cached between calls except `local-vlm`'s loaded weights, inside the warm daemon's own process. The weights path and `cloud-vlm`'s key are still re-checked every call, and the daemon re-validates the requested model against the one it holds.
...
3e. The `local-vlm` warm daemon is a cache, never a dependency, and never silently degrades. Four locks in `src/engines/local-vlm-warm.ts` and `local_vlm_daemon.py`: a protocol/model handshake before any request (mismatch replaces the daemon, never reloads in place); one inference at a time with a queue cap that answers `local_vlm_busy` instead of queueing without bound; idle exit so an abandoned model does not hold RAM; and any start/transport failure returning a named reason that the engine carries into the cold-path result as `warmError`. Covered by `src/test/local-vlm-warm.test.ts` (reuse, idle exit, queue cap, fallback, model swap, protocol mismatch, warm off).
...
- 2026-09-12, new: the `local-vlm` latency numbers (cold and warm both) are for `Qwen2-VL-2B-Instruct-4bit`, not the documented default `Qwen3-VL-8B-Instruct-4bit`, whose weights are not on this machine. The 8B will be materially slower; the number is unmeasured. Every number in this doc was taken on a machine at load average 24-45 — an idle machine is faster (an earlier pass at load 14 gave warm `describe` 4.7-5.9 s), so treat these as a busy-machine ceiling, not a best case.
- **Closed 2026-09-12** (was: `local-vlm`'s ~13 s per image is dominated by cold model load, and nothing kept a process warm): the warm daemon ships in 0.2.2 — warm p50 5.7 s transcribe / 6.7 s describe on the 2B, against 10.8-17.4 s cold in the same session. Still open inside that: warm `describe` is above the 5 s target on this machine's load, and the 8B default is unmeasured, so the model choice is the next thing to revisit rather than the number.
...
| 0.2.2 | 2026-09-12 | `local-vlm` warm daemon: per-user 0600 Unix socket, model loaded once, idle exit, queue cap, `zrv local-vlm status\|stop`, cold path kept as a reason-carrying fallback; warm p50 5.7 s / 6.7 s measured | `feat/warm-local-vlm` |
