...
| `cloud-vlm` | Named explicitly | Real HTTP. One `POST {baseUrl}/chat/completions`, OpenAI-compatible, image as a base64 data URL, `temperature: 0`, bounded `max_tokens`. No SDK — plain `fetch`. Endpoint: `ZRV_CLOUD_VLM_BASE_URL` → config `baseUrl` → `LITELLM_BASE_URL` → `LLM_BASE_URL` → `http://127.0.0.1:8000/v1`. Key: `ZRV_CLOUD_VLM_API_KEY` → the variable named by `ZRV_CLOUD_VLM_KEY_ENV` / config `apiKeyEnv` (default `LITELLM_MASTER_KEY`). Model: `ZRV_CLOUD_VLM_MODEL`, default `gemini-3.7-flash`. Verified live 2026-09-12 through the self-hosted LiteLLM proxy: `transcribe` on `fixtures/ocr/hello.png` returned "HELLO ZEROVISION" in 4.2 s; `describe` was proved on the same proxy with `claude-sonnet-4-6` (5.8 s / 6.0 s on a 1280x800 PNG) after Gemini's own daily quota 429'd. Refuses video and clipboard. 10 MB cap |
